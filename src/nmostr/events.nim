...
proc sysRng32*(data: var openArray[byte]): bool =
  ## Fill `data` with random bytes generated by your operating system.
  ## WARNING: Assumes data to be a 32 byte sequence
  try:
...
proc newKeypair*(rng: Rng = sysRng32): Keypair =
...
proc sign*(event: var Event, sk: SkSecretKey, rng: Rng = sysRng32) {.raises: [ValueError].} =
...
template sign*(event: var Event, sk: Keypair, rng: Rng = sysRng32) =
...
proc stamp*(event: var Event, keypair: Keypair, rng: Rng = sysRng32) {.raises: [ValueError].} =
